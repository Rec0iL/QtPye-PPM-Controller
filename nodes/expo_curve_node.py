...
class CurveVisualizer(QGraphicsItem):
    ...
    def paint(self, painter, option, widget=None):
        ...
        for i in range(int(bounds.width()) + 1):
            ...
            if i == 0:
                path.moveTo(i, y_pixel)
            else:
                path.lineTo(i, y_pixel)
        ...
        painter.setBrush(QColor("#FF5722")) # A bright orange
        painter.setPen(Qt.NoPen)
        painter.drawEllipse(QPointF(x_pixel, y_pixel), 4, 4)

class ExpoCurveNode(BaseNode):
    def __init__(self, x=0, y=0, parent=None):
        super().__init__(title="Expo Curve", x=x, y=y, w=200, h=180, parent=parent)
        self.inputs = 1
        ...
        self.expo_amount = 0.0 # 0% expo by default (linear)
        self.current_input_value = 0.0 # Store current input for the dot
    ...
    def set_value(self, value, input_index=0):
        ...
        self.current_input_value = input_val # Store the value
        ...
        self.visualizer.update() # Tell the visualizer to repaint
    ...
    def get_input_dot_rects(self):
        scene_pos = self.mapToScene(self.input_rects[0].center())
